...
int main(int argc, char *argv[]) {
    int nrows, ncols, rank, nslaves, chunksize, normal_chunksize;
    ...
    if(rank == MASTER) {
        ...
        normal_chunksize = chunksize;
        for(i = 1; i <= nslaves; i++) {

            if(i == nslaves)
                chunksize = ncols/nslaves + ncols % nslaves;
    ...
    // I'M A SLAVE
        ...
        // recv nrows
        MPI_Recv(&nrows, 1, MPI_INT, MASTER, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
